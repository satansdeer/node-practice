...
fixture("Test").page("http://localhost:5000");

test("Test_Landing_Page", async t => {
  await t
    .expect(title.innerText)
    .eql("My Todo List")
    .expect(addTodoButton.innerText)
    .eql("Add ToDo");
});
...
test.skip("New_Todo_Second_Method", async t => {
  ...
  await t
    .typeText(
      // BAD SELECTOR - use data arguments to make elements searchable
      Selector(
        "body > div > div > div.col-md-8 > div > form > div:nth-child(1) > input"
      ),
      "Eat Food",
      { replace: true }
    )
    .typeText(
      Selector(
        "body > div > div > div.col-md-8 > div > form > div:nth-child(2) > input"
      ),
      "3pm",
      { replace: true }
    )
